...
ise_prj_path = "C:\\XilinxProjects\\PhD\\vhdl_libs\\unity_link\\trunk\\prj\\unity_link_absp_wbm_uart\\unity_link_absp_wbm_uart.xise"
planahead_prj_path = "C:\\XilinxProjects\\PhD\\lib_test\\planahead_test\\planahead_test.ppr"

libs = []
libs.append(["uart", "v02_00_a"])
libs.append(["wishbone", "v03_50_a"])
libs.append(["utility", "v01_00_a"])
libs.append(["checksum", "v02_00_a"])
libs.append(["pulse_gen", "v02_00_a"])
libs.append(["rr_prio_enc", "v02_00_a"])
libs.append(["dual_fifo_mux", "v02_00_a"])
libs.append(["registers", "v02_00_a"])
...
use_64_bit = True
copy_all_sources = False
force_copy_on_self_inclusion = False
test_for_import_directives = False
...
class lib_source(object):
    ...
    def __init__(self, name, parent):
        ""
        self.dependencies = []
        self.force_copy = copy_all_sources
    ...
    def update_dependencies(self):
        ""
        own_package = self.parent.parent.name
        own_version = self.parent.name
        ...
        for line in f:
            ...
            elif test_for_import_directives==True and l.startswith("--#force_copy") == True:
                self.force_copy = True
            elif test_for_import_directives==True and l.startswith("--#force_work") == True:
        ...
        for package, version in dependency: 
            "test inclusion of own package_version to avoid endless loop"
            if own_package != package or own_version != version:
                ...
                "self-inclusion"
                if force_copy_on_self_inclusion==True:
                    self.force_copy = True
...
def import_list_builder(collection, libs, im={}):
    """
        ...
        returns:    : {lib_name: [file_path, force_copy, force_work]}
                        
                        lib_name : package_name + version_name
                        file_path : full path of file including the file name
                        force_copy : boolean, if true source must be copied from repository to local project library
                        force_work : boolean, if true source must be copied from repository to project work-library
    """
    for l in libs:
        key = l[0]+"_"+l[1] 
        if key not in im:
                    ...
                    for version in pkg.versions:
                        if version.name == l[1]:
                            ...
                            for s in version.sources:
                                ...
                                files.append([s.path, s.force_copy, s.force_work])
                            im[key] = files
...
def generate_ise_tcl_script(xise_file, import_map):
    ...
    for key in import_map:
        lib = key
        file_data = import_map[key]        
        sl.append("lib_vhdl new " + lib)
        for file_path, force_copy, force_work in file_data:
            if force_work==True:
                sl.append("xfile add " + file_path + " -copy " + """-view "All" """ )
            elif force_copy==True:
                filename = file_path[file_path.rfind('/')+1:len(file_path)]
                sl.append("xfile add " + file_path + " -copy " + """-view "All" """ )
                sl.append("lib_vhdl add_file " + lib + " " + filename)
            else:
                sl.append("xfile add " + file_path + " -lib_vhdl " + lib + """ -view "All" """ )
...
def generate_planahead_tcl_script(ppr_file, import_map):
    ...
    for key in import_map:               
        lib = key
        files = import_map[key][0]
      
        #TODO: implement copy file feature  
        
        add_files = "add_files -norecurse {"
        set_property = "set_property library " + lib + " [get_files {"        
                     
        for file_path in files:
            add_files = add_files + file_path + "\n"
            set_property = set_property + file_path + "\n"
